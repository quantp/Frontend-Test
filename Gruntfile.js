module.exports = function(grunt) {
    ...
    concat: {
      dist: {
        files: [{
          ...
          '<%= meta.build %>js/script.js': ['<%= meta.assets %>js/settings.js']
        }]
      }
    },
    copy: {
      css: {
        files: [{
          ...
          cwd: '<%= meta.assets %>css/',
          src: '**/*',
          dest: '<%= meta.build %>css/<%= meta.assets %>css'
        }]
      },
      data: {
        files: [{
  ...
  grunt.registerTask('build', ['clean', 'concat', 'pug', 'less', 'copy', 'htmlhint', 'jshint']);
